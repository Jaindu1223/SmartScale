...
def prepare_lstm_input(cloudwatch_values, sequence_length=15):
    ...
    """
    # 1. We need enough history to form a sequence
    if len(cloudwatch_values) < sequence_length:
        return None, None
        
    # 2. Reshape into a 2D array for the Scaler
    ...
    # 4. Extract only the most recent sequence for a live prediction
    recent_sequence = scaled_data[-sequence_length:]
    
    # 5. Reshape into 3D for Keras: (batch_size, time_steps, features)
